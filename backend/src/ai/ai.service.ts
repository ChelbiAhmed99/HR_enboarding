...
const DOC_TYPE_MAP: Record<string, { type: string; fields: string[]; requiredFields: string[] }> = {
  CIN: {
    type: 'CARTE_IDENTITE',
...
const CHATBOT_KB: Record<string, { patterns: string[]; response: string; suggestions: string[] }[]> = {
  EMPLOYEE: [
    {
...
const DEFAULT_RESPONSES: Record<string, { response: string; suggestions: string[] }> = {
  EMPLOYEE: {
...
export class AiService {
  ...
  analyzeDocument(name: string, type: string): DocumentAnalysis {
    ...
    // Simulate intelligent confidence scoring based on document type
    const baseConfidence = 0.78 + Math.random() * 0.20; // 78% - 98%
    const confidence = Math.round(baseConfidence * 100) / 100;

    // Simulate extracted vs missing fields
    const extractedFields = [...config.fields];
    const missingFields: string[] = [];

    // Randomly decide if some fields are "missing" (10% chance per optional field)
    const optionalFields = config.fields.filter(
      (f) => !config.requiredFields.includes(f),
    );
    for (const field of optionalFields) {
      if (Math.random() < 0.1) {
        missingFields.push(field);
        const idx = extractedFields.indexOf(field);
        if (idx > -1) extractedFields.splice(idx, 1);
      }
    }
  ...
  async saveAnalysis(documentId: string, analysis: DocumentAnalysis) {
  ...
  /* ─── 3. Chatbot ────────────────────────────────────── */
  async chatResponse(question: string, userId: string): Promise<AiChatResponse> {
    // Determine user role
  ...
  async generateInsights(): Promise<AiInsight[]> {
    ...
    // Fetch real data for intelligent insights
    const [
      totalOnboardings,
      inProgressCount,
      completedCount,
    ...
    // Insight 5: General recommendation
